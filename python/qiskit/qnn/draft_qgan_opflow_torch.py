...
class QGAN(torch.nn.Module):
    def __init__(self, circ_real, circ_gen, circ_disc):
        ...
        self.qnn_gen = qiskit_machine_learning.neural_networks.OpflowQNN(self.op_gen_disc,
                    theta_d, theta_g, opflow_pauli_exp, opflow_gradient, qi_aer_statevector)
        # disc wants to maximize this expval
        self.qnn_fake_disc = qiskit_machine_learning.neural_networks.OpflowQNN(self.op_gen_disc,
                    theta_g, theta_d, opflow_pauli_exp, opflow_gradient, qi_aer_statevector)
        # disc wants to minimize this expval
        self.qnn_real_disc = qiskit_machine_learning.neural_networks.OpflowQNN(self.op_real_disc,
                    [], theta_d, opflow_pauli_exp, opflow_gradient, qi_aer_statevector)
    ...
    def backward_disc(self):
        ...
        grad_fake = self.qnn_fake_disc.backward(theta_gen_np, theta_disc_np)[1][0,0]
        grad_real = self.qnn_real_disc.backward([], theta_disc_np)[1][0,0]
        self.theta_disc.grad.data += torch.tensor(grad_real - grad_fake, dtype=self.theta_disc.dtype)

    def forward_disc(self):
        theta_gen_np,theta_disc_np = self.get_theta_np()
        prob_fake = (1-self.qnn_fake_disc.forward(theta_gen_np, theta_disc_np)[0,0])/2
        prob_real = (1-self.qnn_real_disc.forward([], theta_disc_np)[0,0])/2
        return prob_fake,prob_real

    def backward_gen(self):
        theta_gen_np,theta_disc_np = self.get_theta_np()
        grad = self.qnn_gen.backward(theta_disc_np, theta_gen_np)[1][0,0]
        self.theta_gen.grad.data += torch.tensor(grad, dtype=self.theta_gen.dtype)

    def forward_gen(self):
        theta_gen_np,theta_disc_np = self.get_theta_np()
        prob_fake = (1-self.qnn_gen.forward(theta_disc_np, theta_gen_np)[0,0])/2
        return prob_fake
