...
for ind_epoch in range(10):
    with tqdm(total=len(trainloader), desc='epoch-{}'.format(ind_epoch)) as pbar:
        ...
        for ind_batch, (data_i, label_i) in enumerate(trainloader):
            data_i, label_i = data_i.to(device).half(), label_i.to(device)
            with torch.cuda.amp.autocast():
                optimizer.zero_grad()
                predict = net(data_i) #float16
                loss = F.cross_entropy(predict, label_i) #float32
            loss_scaler.scale(loss).backward() #net.fc1.weight.grad.dtype #float32
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                tmp0 = ((net(x.to(device).half()), y.to(device)) for x,y in testloader)
